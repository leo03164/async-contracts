...
contract AsyncArtwork is ERC721, ERC721Enumerable, ERC721Metadata {
    ...
    function updateRoyaltyPercentages(
        ...
        require(
            _platformSecondSalePercentage.add(_artistSecondSalePercentage) < 100
        );
    ...
    function mintArtwork(
        ...
        string memory artworkTokenURI,
        address payable[] memory controlTokenArtists
    ) public onlyWhitelistedCreator {
        require(
            artworkTokenId == expectedTokenSupply,
            "ExpectedTokenSupply different"
        );
        ...
        for (uint256 i = 0; i < controlTokenArtists.length; i++) {
            ...
            if (controlTokenArtists[i] != msg.sender) {
                ...
                for (
                    uint256 k = 0;
                    k < uniqueTokenCreators[artworkTokenId].length;
                    k++
                ) {
                    if (
                        uniqueTokenCreators[artworkTokenId][k] ==
                ...
                if (containsControlTokenArtist == false) {
                    uniqueTokenCreators[artworkTokenId].push(
                        controlTokenArtists[i]
                    );
                }
            }
    ...
    function bid(uint256 tokenId) public payable {
        ...
        if (pendingBids[tokenId].exists) {
            ...
            require(
                msg.value > pendingBids[tokenId].amount,
                "Bid must be > than current bid"
            );
    ...
    function onTokenSold(uint256 tokenId, uint256 saleAmount, address to)
        private
    {
        ...
        if (tokenDidHaveFirstSale[tokenId]) {
            ...
            uint256 platformAmount = saleAmount
                .mul(platformSecondSalePercentage)
                .div(100);
            ...
            uint256 creatorAmount = saleAmount
                .mul(artistSecondSalePercentage)
                .div(100);
            ...
            uint256 platformAmount = saleAmount
                .mul(platformFirstSalePercentage)
                .div(100);
            platformAddress.transfer(platformAmount);
        ...
        returns (int256[] memory)
    {
        ...
        int256[] memory returnValues = new int256[](
            controlToken.numControlLevers.mul(3)
        );
    ...
    function useControlToken(
        ...
        for (uint256 i = 0; i < leverIds.length; i++) {
            ...
            ControlLever storage lever = controlTokenMapping[controlTokenId]
                .levers[leverIds[i]];
